...
# save group and read it in
     ...
     p1 <- ggplot(comp_sim2, mapping = aes(x = time, y = cort, by = animal_sample, color = type)) +
       ...
       scale_color_manual(values = c("#9D93DE", "#FDAE42")) +
       #guides(color = "none") +
       theme(axis.title = element_text(size = 14)) +
       annotate(geom = "text", x = -Inf, y = Inf, hjust = -.5, vjust = 1.5, label = "A") +
...
# save group and read it in
    set.seed(61)
     ...
     p1 <- ggplot(comp_sim2, mapping = aes(x = time, y = cort, by = animal_sample, color = type)) +
       ...
       guides(color = "none") +
       theme(axis.title = element_text(size = 14)) +
       geom_smooth(se = FALSE, size = 1.1, mapping = aes(by = type, color = type)) +
       theme(legend.title = element_blank())
...
# sampling scheme 1
     ...
     p1a <- p1 +
       ...
       geom_smooth(data = dd1, mapping = aes(x = time, y = cort, by = type, color = type, fill = type), linetype = "dashed") +
       scale_fill_manual(values = c("slateblue", "orange")) +
       annotate(geom = "text", x = -Inf, y = Inf, hjust = -.5, vjust = 1.5, label = "A") +
       guides(fill = "none") +
       geom_point(data = dd1, mapping = aes(x = time, y = cort, color = type, fill = type), alpha = 0.6) +
       ggtitle("Standard sampling")
...
# sampling scheme 2
     ...
     for(i in 1:nrow(ans)){
       ...
       subs <- sub[sample(seq(1, 60, 1), 2, replace = FALSE), ]
       dd2 <- rbind(dd2, subs)
     }
     ...
     p1b <- p1 +
       ...
       geom_smooth(data = dd2, mapping = aes(x = time, y = cort, by = type, color = type, fill = type), linetype = "dashed") +
       scale_fill_manual(values = c("slateblue", "orange")) +
       annotate(geom = "text", x = -Inf, y = Inf, hjust = -.5, vjust = 1.5, label = "B") +
       guides(fill = "none") +
       ylab("") +
       geom_point(data = dd2, mapping = aes(x = time, y = cort, color = type, fill = type), alpha = 0.6) +
...
# sampling scheme 3
     ...
     p1c <- p1 +
       ...
       geom_smooth(data = dd3, mapping = aes(x = time, y = cort, by = type, color = type, fill = type), linetype = "dashed", span = 0.9) +
       scale_fill_manual(values = c("slateblue", "orange")) +
       scale_color_manual(values = c("slateblue", "orange")) +
